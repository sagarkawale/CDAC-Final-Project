...
function DataAdder() {
  ...
  const postData = async () => {
    const url = 'http://localhost:3003/api/admin/products/';
    ...
    mens_kurta.forEach(item => {
        item.price = parseInt(item.selling_price.replace('₹', '').replace(',', '')); // Convert price string to integer
        delete item.selling_price; // Remove the selling_price property
        item.size= [
            {
              "name": "S",
              "quantity": 20
    ...
    for (let i = 0; i < mens_kurta.length; i++) {
        mens_kurta[i].topLavelCategory= "Women";
        mens_kurta[i].secondLavelCategory="Dress";
        mens_kurta[i].thirdLavelCategory="Kurta";
    }
    mens_kurta.map(async (jsonData, index) => {
        try {
